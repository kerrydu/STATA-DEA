...
*注意energy要放在第三个投入位置
capture program drop EEIg_supeff
program define EEIg_supeff, rclass
    version 12.1
...
qui {
	...
	mat mmat=Xmat[1..(`nob'-1),....]
	}
	else {
	mat mmat=Xmat[1..(`i'-1),....]
	mat mmat=[mmat \ Xmat[(`i'+1)..`nob',....]]
	}
    mat m2=[obj,mmat]
    mat m2=m2'
	   *preserve
	  ...
	  * mat list Xmat
		 ...
		 * mat temp4=fobj[1,2..6]
		  mat eff[`i',1]=0.5*(2-fobj[1,2]-fobj[1,4])/(1+fobj[1,3])
		  *list rel
		   *mat dir 
		   restore
		  }
