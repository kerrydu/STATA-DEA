	*! version 1.0.1 
	*Ã»ÓÐ¿¼ÂÇÊ±Ðò
capture program drop TEP2
program define TEP2, rclass
    version 12.1
...
/*
	...
	*/
	...
	foreach j in Tbetween TFEE MEPI EFFCH TECCH {
	cap drop `j'
	}
	...
	*local j=1
	...
	forvalues i=1/`T' {
	   ...
	   *keep `gopvars' `bopvars' `invars'
	   ...
	   mkmat `outvars' `invars' if `tid'==`i', mat(Xmat)
	   mkmat `outvars' `invars' if `tid'==`i', mat(Xmat0)
	  ...
	  * mat list Xmat
	  /*
		...
		mat dmutemp2=-Xmat0[`j',....]
		mat dmutemp2[1,1]=J(1,`nout'+`ninp'-1,0)
		mat dmutemp2=[1 \ dmutemp2']
		
		mat dmutemp1=Xmat0[`j',....]
		mat dmutemp1[1,`nout'+`ninp']=0
		mat dmutemp1=[0 \ dmutemp1']
...
qui {
	...
	*local i=2
	   ...
	   *keep `gopvars' `bopvars' `invars'
	   ...
	   mkmat `outvars' `invars' if `tid'==`i', mat(Xmat)
	   mkmat `outvars' `invars' if `tid'==`i'-1, mat(Xmat0)
...
qui keep `dmu' Tbetween MEPI EFFCH TECCH
order `dmu' Tbetween MEPI EFFCH TECCH
disp _n(2) "Malmquist energy productivity index(MEPI) and its decomposition"
list, sep(0) 
restore
qui{
bys `dmu': gen Tbetween1=Tbetween[_n-1]
bys `dmu': gen MEPI1=MEPI[_n-1]
bys `dmu': gen EFFCH1=EFFCH[_n-1]
bys `dmu': gen TECCH1=TECCH[_n-1]
drop Tbetween MEPI EFFCH TECCH
