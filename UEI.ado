...
capture program drop UEI
program define UEI, rclass
    version 12.1
...
qui {
	...
	*mkmat `gopvars' `bopvars' `invars' if `tid'==`i', mat(Xmat)
	mkmat `invars' `gopvars' `bopvars' if `tid'==`i', mat(Xmat)
	  ...
	  forvalues j=1/`nobs' {
		 ...
		 * mat temp4=fobj[1,2..6]
		  mat eff[`k',1]=0.25*(4-fobj[1,2]-fobj[1,3]-fobj[1,4]-fobj[1,6])/(1+fobj[1,5])
		  /*
		  if `k'==24 {
		  noi mat list fobj
		  noi mat list XZ
		  save bug2015,replace
		  }
		  */
		  local ++k
