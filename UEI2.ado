...
*注意与UEIg 矩阵变量排序不一致
*加非负约束;2015.2.19
capture program drop UEI2
program define UEI2, rclass
    version 12.1
		...
		*/  
		/*  
		 forvalues iter=1/`ninp' {
		   mat eff[`k',1]=eff[`k',1]+(1-fobj[1,1+`iter'])
		   }
		 forvalues iter=1/`nbo' {
		    mat eff[`k',1]=eff[`k',1]+(1-fobj[1,1+`ninp'+`ngo'+`iter'])
			}
		 mat eff[`k',1]=eff[`k',1]/(`ninp'+`nbo')/(1+fobj[1,1+`ninp'+1])
		 */
		 local dn=`ninp'+`nbo'
		 forvalues iter=1/`dn' {
		    mat eff[`k',1]=eff[`k',1]+(1-fobj[1,1+`ngo'+`iter'])
			}
		mat eff[`k',1]=eff[`k',1]/(`ninp'+`nbo')/(1+fobj[1,2])
