...
*×¢ÒâÓëUEIg ¾ØÕó±äÁ¿ÅÅÐò²»Ò»ÖÂ
*¼Ó·Ç¸ºÔ¼Êø;2015.2.19
capture program drop bglpindex
program define bglpindex, rclass
    version 12.1
...
*local i=1
	*****
	local tpred=`i'-1
	mkmat `gopvars' `bopvars' `invars' if `tid'==`i'|`tid'==`tpred', mat(Xmat)
	mkmat `gopvars' `bopvars' `invars' if `tid'==`i', mat(Xmat0)

	count if `tid'==`i'
	local nobs=r(N)
	count if `tid'==`i'|`tid'==`tpred'
	local mobs=r(N)

	   cap mat drop m1 m2 obj m3 fobj temp2 temp3  XZ
	   mat obj=J(`mobs',1,0)
       mat m2=[obj,Xmat]
       mat m2=m2'
	  forvalues j=1/`nobs' {
...
cap drop Dfun1	
svmat eff, names(Dfun)

qui bys `dmu' (`time'): gen bglpindex=Dfun1[_n-1]-Dfun1[_n] if _n>1
