...
**¼Ó·Ç¸ºÔ¼Êø;2015.2.19
capture program drop gglpindex
program define gglpindex, rclass
    version 12.1
...
// -----------------------------------------------------------------------------
    ...
    syntax varlist(min=1) [if] [in], dmu(varname) time(varname)
...
qui bys `dmu' (`time'): gen gglpindex=Dfun1[_n-1]-Dfun1[_n] if _n>1
